...
function pm_article_type_listing_shortcode_fn(array $atts)
{
    ...
    $article_type_label = is_object($qo) && isset($qo->name) ? $qo->name . ' ' . $article_type_label : $article_type_label;

    $output .=
        '<div class="' . $class . '" id="' . $id . '">
            <div class="large-12 cell articles-header ' . $maybeHideForMobile . '">';
    $output .= is_object($qo) && isset($qo->name) && $article_type_slug === 'features'
        ? '<h1><a href="' . $more_link . '" class="float-left hed">' . $article_type_label . '</a></h1>'
