...
ERR;
    ...
    private function importSettings($import_data)
    {
        ...
        $global = DUP_PRO_Global_Entity::getInstance();
        $global->setFromData($import_data->settings);

        if (!property_exists($import_data, 'secure_settings')) {
            return true;
        }
        $sglobal = DUP_PRO_Secure_Global_Entity::getInstance();
        $sglobal->setFromData($import_data->secure_settings);
        $sglobal->save();
     ...
     *
     * @param object $import_data
     * @param int[]  $storage_map
     * @param int[]  $template_map
     *
     * @return void
     */
    private function importSchedules($import_data, $storage_map, $template_map)
    {
        ...
        foreach ($import_data->schedules as $schedule_data) {
            ...
            for ($i = 0; $i < count($schedule->storage_ids); $i++) {
                ...
                if (isset($storage_map[$old_storage_id])) {
                    $schedule->storage_ids[$i] = $storage_map[$old_storage_id];
                }
            }

            if (isset($template_map[$schedule->template_id])) {
