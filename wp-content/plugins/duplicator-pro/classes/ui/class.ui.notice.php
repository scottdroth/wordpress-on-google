...
class DUP_PRO_UI_Notice
{
    ...
    public static function clearInstallerFilesAction()
    {
        ...
        if (SnapUtil::filterInputRequest('action', FILTER_DEFAULT) === 'installer') {
            ...
            <div id="message" class="notice notice-success">
                <?php require DUPLICATOR____PATH . '/views/parts/migration-clean-installation-files.php'; ?>
            </div>
            <?php
        }
    }
    ...
    public static function migrationSuccessNotice()
    {
        ...
        if (DUP_PRO_CTRL_Tools::isDiagnosticPage()) {
            require DUPLICATOR____PATH . '/views/parts/migration-message.php';
        } else {
            require DUPLICATOR____PATH . '/views/parts/migration-almost-complete.php';
        }
    }
    ...
    public static function showNoExportCapabilityNotice()
    {
        $errorMessage = DUP_PRO_U::__(
            '<strong>Duplicator Pro</strong><hr> Your logged-in user role does not have export capability so you don\'t have access to Duplicator Pro functionality.'
        ) .
            "<br>" .
            sprintf(
                DUP_PRO_U::__('<strong>RECOMMENDATION:</strong> Add export capability to your role. See FAQ: <a target="_blank" href="%s">%s</a>'),
                'https://snapcreek.com/duplicator/docs/faqs-tech/#faq-licensing-040-q',
                DUP_PRO_U::__('Why is the Duplicator/Packages menu missing from my admin menu?')
            );
