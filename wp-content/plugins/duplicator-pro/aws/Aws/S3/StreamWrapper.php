...
class StreamWrapper
{
    ...
    public function stream_open($path, $mode, $options, &$opened_path)
    {
        ...
        if (strpos($mode, '+')) {
            $errors[] = 'The S3 (or S3 Compatible) stream wrapper does not allow simultaneous reading and writing.';
        }
        ...
        if ($mode == 'x' && static::$client->doesObjectExist($params['Bucket'], $params['Key'], $this->getOptions())) {
            $errors[] = "{$path} already exists on S3 (or S3 Compatible)";
        }
    ...
    public function rename($path_from, $path_to)
    {
        ...
        if (!$partsFrom['Key'] || !$partsTo['Key']) {
            return $this->triggerError('The S3 (or S3 Compatible) stream wrapper only supports copying objects');
        }
